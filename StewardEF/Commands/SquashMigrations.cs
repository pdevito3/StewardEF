...
internal class SquashMigrationsCommand : Command<SquashMigrationsCommand.Settings>
{
    ...
    private static void ReplaceWithSqlScript(string migrationFilePath, string upSql, string downSql)
    {
        ...
        upSql = SanitizeEfGeneratedSql(upSql);
        downSql = SanitizeEfGeneratedSql(downSql);
...
{downSql.Replace("\"", "\"\"")}        "");";
    ...
    /// <summary>
    /// Sanitizes SQL generated by 'dotnet ef migrations script' for use in migrationBuilder.Sql().
    /// Removes transaction statements and EF migrations history statements that conflict with
    /// EF Core's automatic transaction handling and history tracking.
    /// </summary>
    internal static string SanitizeEfGeneratedSql(string sql)
    {
        // Remove transaction statements (EF Core wraps migrations in its own transaction)
        // Handles: START TRANSACTION, BEGIN TRANSACTION, BEGIN, COMMIT, END (for some providers)
        ...
        sql = Regex.Replace(sql,
            @"^\s*INSERT\s+INTO\s+[\[\]""']?[\w.]*[\[\]""']?__EFMigrationsHistory[\[\]""']?\s*\([^)]+\)[\s\S]*?VALUES[\s\S]*?\([^)]+\)\s*;\s*$",
            string.Empty, RegexOptions.Multiline | RegexOptions.IgnoreCase);

        // Remove DELETE FROM __EFMigrationsHistory (EF Core tracks this automatically)
        // Handles various quote styles: "", '', [], none, and table name with/without schema
        sql = Regex.Replace(sql,
            @"^\s*DELETE\s+FROM\s+[\[\]""']?[\w.]*[\[\]""']?__EFMigrationsHistory[\[\]""']?\s*WHERE[^;]+;\s*$",
            string.Empty, RegexOptions.Multiline | RegexOptions.IgnoreCase);
